...
function RevealImageListItem({ text, images }: ShowImageListItemProps) {
  const imageContainerBase = "absolute top-1/2 -translate-y-1/2 z-10 h-40 w-40 overflow-hidden rounded-md shadow-xl transition-all duration-300 ease-in-out opacity-0 group-hover:opacity-100 scale-90 group-hover:scale-100";

  return (
    <div className="group relative w-full cursor-pointer border-b border-border/20 py-8">
      <h1 className="text-5xl font-black text-left text-foreground transition-all duration-300 group-hover:text-accent">
          {text}
      </h1>
      <div
        className={cn(
